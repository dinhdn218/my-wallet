...
export function ConTieuDuoc({
  ...
  const truoc = !t.coHanMuc ? t.daTieu : t.over ? t.hanMuc - t.daTieu : t.conLai
      ...
      {hasHydrated ? (
        <p
          ...
          className={cn(
            ...
            compact ? 'text-[clamp(38px,9vw,52px)]' : 'text-[clamp(40px,7vh,72px)]',
        ...
        <AmountSkeleton className={cn('mt-2 w-[80%]', compact ? 'h-[clamp(38px,9vw,52px)]' : 'h-[clamp(40px,7vh,72px)]')} />
      )}
        ...
        (t.coHanMuc ? (
          <>
            <p className="mt-2.5 font-mono text-[11px] leading-relaxed text-muted">
              {!compact && (
                <>
                  Đã tiêu <b className="font-bold text-foreground">{formatVnd(t.daTieu)}</b>{' '}
                  trên <b className="font-bold text-foreground">{formatVnd(t.hanMuc)}</b>
                  <br />
                </>
              )}
              Còn <b className="font-bold text-foreground">{t.soNgayConLai} ngày</b>
              {t.over ? (
                <> · đã vượt <b className="font-bold text-negative">{formatVnd(t.daTieu - t.hanMuc)}</b></>
              ) : (
                <> · khoảng <b className="font-bold text-foreground">{formatVnd(t.moiNgay)}</b> mỗi ngày</>
              )}
            </p>
